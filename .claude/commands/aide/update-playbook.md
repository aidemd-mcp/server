...
Help the user add new conventions, modify existing ones, or reorganize sections in their coding playbook. Every invocation ends with a required drift-detection step that keeps the playbook entry-point's task routing table in sync with the actual section structure.
...
- [ ] Use the `study-playbook` skill to read the playbook entry-point and identify the relevant section (or confirm no section yet exists for the new convention)
...
- [ ] **Housekeeping step (required — do not skip):** Compare the playbook entry-point's task routing table against the actual sections that now exist:

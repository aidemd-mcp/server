 ...
 *   backend-agnostic remediation sentence directing the user to a specific
 *   terminal command or file edit. `instructions` is never empty on any branch.
 * - No `backend`, `name`, or `connector` field on any branch — consumers branch
 *   on `status` alone.
...
export default async function brain(root: string): Promise<BrainToolResult> {
	...
	// is a missing-or-broken-file shape, so NO_BRAIN_AIDE_PROSE is the natural
	// remediation — it directs the user to run init, which scaffolds the file.

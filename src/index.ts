...
export async function routeSubcommand(): Promise<boolean> {
	...
	if (process.argv[2] === "sync") {
		const { runSync } = await import("./cli/sync/index.js");
		if (process.argv.includes("--help")) {
			process.stdout.write(
				"Usage: npx aidemd-mcp sync\n\n" +
					"Reads `.aide/config/brain.aide` from the current project root, derives the brain\n" +
					"MCP server entry by interpolating the configured brain root path into the\n" +
		...
		},
		{
			...
			inputSchema: {
				...
				properties: {
					...
					brainPath: {
						type: "string",
						description: "Resolved brain root path. Required when category=brain. The agent provides this after interviewing the user.",
					},
				},
			},
		},

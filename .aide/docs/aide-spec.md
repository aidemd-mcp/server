...
The name is a double entendre. AIDE is also an **AI Domain Expert** — a research agent that can shoulder some of the domain-knowledge burden for an engineering team. But this is a perk, not the point. Intent is the first-class citizen; research is just one way to inform it. And the research layer only makes sense for low-risk domains — content generation, marketing copy, SEO strategy, that sort of thing. High-stakes fields like law and medicine still need real experts; an LLM summarizing statutes or clinical guidelines is not a substitute for licensed professionals who carry liability. Where the domain fits, though, the research agent reads sources, synthesizes findings, and persists them to the brain as durable knowledge the planner can draw from when writing the `.aide` spec. The engineer doesn't need to become a content-strategy expert. The team doesn't need to hire one. The brain's research plus the planner's intent spec together *are* the expertise — for domains where that's appropriate.

The research layer is optional. It runs only when the module needs domain knowledge the team doesn't already have. If a research agent already filled the brain, or the user holds the knowledge directly, the planner skips research and writes the intent spec from what's available. Once the intent doc exists, the rest of the methodology treats it as authoritative regardless of how the planner got there.
...
1. **Planner agent** (`/aide:spec`). Interviews the user and produces the `.aide` frontmatter only — `scope`, `intent`, `outcomes.desired`, `outcomes.undesired`. No body sections, no research, no code. The planner captures what the user wants; downstream agents figure out how. The frontmatter is presented to the user for confirmation before proceeding.
